...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        uint32_t num_envs = argc > 1 ? (uint32_t)strtoul(argv[1], NULL, 10) : 2359296u;
        uint32_t steps = argc > 2 ? (uint32_t)strtoul(argv[2], NULL, 10) : 500u;
        uint32_t iterations = argc > 3 ? (uint32_t)strtoul(argv[3], NULL, 10) : 14u;
        float sigma = argc > 4 ? strtof(argv[4], NULL) : 0.25f;
        float learning_rate = argc > 5 ? strtof(argv[5], NULL) : 0.3f;
        ...
        printf("metal_es_train pendulums=2 features=%u weights=%u threadgroup=%u steps=%.0f elapsed=%.6fs sps=%.0f weights_checksum=%.6f first_weight=%.6f\n",
               FEATURE_COUNT, WEIGHT_COUNT, THREADGROUP_SIZE, total_steps, elapsed, steps_per_second, w[0], w[0]);
        if (output_path != NULL) {
